...
/**
 * Created by rayoc on 24/01/2017.
 */
public class PantallaJuego implements Screen
{
    ...
    private Texto texto; //Para poner mensajes en pantalla

    //Estado del juego
    private Estado estado = Estado.JUGANDO; //Jugando,pausado,finalizado,etc.
    ...
    @Override
    public void show() {
        //Al mostrarse esta pantalla, crear la cámara
    ...
    public void render(float delta) {
        if(estado==Estado.JUGANDO){
            //Actualizar los objetos de la pantalla
        ...
        if(estado==Estado.JUGANDO && pelota.sprite.getX()<=0){
            puntosMaquina++;
            if (estado==Estado.JUGANDO && puntosMaquina>=5){
                //Perdió el jugador
        ...
        //Dibuja la pantalla aprox. 60 veces por segundo
        //Borrar la pantalla
        Gdx.gl.glClearColor(0,0,0,1); //Blanco opaco
        Gdx.gl.glClear(GL20.GL_COLOR_BUFFER_BIT);
        batch.setProjectionMatrix(camara.combined);
        batch.begin();
        //Aqui dibujaroemos los elementos del juego
        ...
        //Dibuja la pelota
        pelota.dibujar(batch); //Llamarlo dentro de batch.begin()-batch.end();
        //Raquetas
        ...
        //Perdió, reiniciar.
        if(estado!=Estado.JUGANDO){
            texto.mostrarMensaje(batch, "Lo siento, perdiste", 3*ANCHO/4,ALTO/2);
